  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const [agentsRes, employeesRes, hrsRes] = await Promise.all([
        axios.get("https://insurai-backend-production.up.railway.app/agent", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get("https://insurai-backend-production.up.railway.app/auth/employees", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        axios.get("https://insurai-backend-production.up.railway.app/hr", {
          headers: { Authorization: `Bearer ${token}` },
        })
      ]);
  ...
  const fetchAllClaims = useCallback(async () => {
    try {
      ...
      const [claimsRes, empRes, hrRes, policyRes] = await Promise.all([
        fetch("https://insurai-backend-production.up.railway.app/admin/claims", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch("https://insurai-backend-production.up.railway.app/auth/employees", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch("https://insurai-backend-production.up.railway.app/hr", {
          headers: { Authorization: `Bearer ${token}` },
        }),
        fetch("https://insurai-backend-production.up.railway.app/admin/policies", {
          headers: { Authorization: `Bearer ${token}` },
        })
      ]);
...
const handleRegisterAgent = async (agentData) => {
  try {
    const token = localStorage.getItem("token");
    await axios.post("https://insurai-backend-production.up.railway.app/admin/agent/register", agentData, {
      headers: { Authorization: `Bearer ${token}` },
    });
