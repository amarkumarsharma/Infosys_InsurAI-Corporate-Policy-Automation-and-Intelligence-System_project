...
export default function HRDashboard() {
  ...
  const fetchEmployees = useCallback(async () => {
    try {
      ...
      const response = await fetch("https://insurai-backend-production.up.railway.app/auth/employees");
  ...
  const fetchHRList = useCallback(async () => {
    try {
      const response = await fetch("https://insurai-backend-production.up.railway.app/hr");
  ...
  const fetchPolicies = useCallback(async () => {
    try {
      ...
      const response = await fetch("https://insurai-backend-production.up.railway.app/employee/policies", {
  ...
  const fetchClaims = useCallback(async () => {
    try {
      ...
      const response = await fetch(`https://insurai-backend-production.up.railway.app/hr/claims?hrId=${loggedInHrId}`, {
  ...
  const approveClaim = async (id, remarks) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`https://insurai-backend-production.up.railway.app/hr/claims/approve/${id}`, {
        method: "POST",
        headers: { 
  ...
  const rejectClaim = async (id, remarks) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`https://insurai-backend-production.up.railway.app/hr/claims/reject/${id}`, {
        method: "POST",
        headers: { 
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json"
  ...
  const downloadCSV = () => {
    ...
    const rows = displayedClaims.map(c => [
      ...
      c.priority,
      c.daysPending,
      c.documents?.length > 0 ? c.documents.map(d => `https://insurai-backend-production.up.railway.app${d}`).join(" | ") : "No documents"
    ]);
