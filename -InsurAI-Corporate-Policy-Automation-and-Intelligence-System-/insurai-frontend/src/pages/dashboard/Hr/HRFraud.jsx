...
const HRFraud = () => {
  ...
  const fetchFraudAlerts = async () => {
    try {
      ...
      const response = await fetch("https://insurai-backend-production.up.railway.app/hr/claims/fraud", {
        headers: {
...
const fetchEmployees = async () => {
  try {
    const token = localStorage.getItem("token");
    const res = await fetch("https://insurai-backend-production.up.railway.app/employees", {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
                    ...
                    <tr key={a.id} onClick={() => setViewingAlert(a)} className="cursor-pointer">
                      ...
                      <td>
                        {a.documents?.map((doc, i) => (
                          <a key={i} href={`https://insurai-backend-production.up.railway.app${doc}`} target="_blank" className="d-block">
                            Doc {i + 1}
                          </a>
                        ))}
                      </td>
              ...
              <div className="modal-body">
                ...
                <ul>
                  {viewingAlert.documents?.map((doc, i) => (
                    <li key={i}><a href={`https://insurai-backend-production.up.railway.app${doc}`} target="_blank">Document {i + 1}</a></li>
                  ))}
                </ul>
              </div>
